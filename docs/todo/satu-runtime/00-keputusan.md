...
Semua diukur 7 September 2026 dengan stack `erp-dev` (`start.ps1 -Apps management-aset`), idle setelah 40 request pemanasan, container `docs` tidak dihitung.
...
Kode modul 2,6% dari yang dimuat satu proses Laravel. Sisanya framework yang identik di tiap container app. Di satu runtime, framework dimuat sekali dan tiap modul menambah sekitar 2 MB.
...
| Keadaan | Container | RAM |
| --- | --- | --- |
| Desain saat ini, database terpisah | 8 | 275 MiB |
| Satu server Postgres, API aset masih proses terpisah | 7 | 275 MiB |
| Satu runtime, proyeksi | 5 | sekitar 225 MiB |
...
Kode `app-erp-management-aset` memanggil Core secara sinkron saat membuat dokumen: `number-sequences/{ref}/issue` untuk setiap nomor, `fiscal-periods` untuk validasi tanggal, `units-of-measure/resolve` untuk baris barang, dan `workflow-instances` bila ada approval. Tiga lompatan per dokumen adalah angka yang dipakai di proyeksi.

## Proyeksi production: angka lokal dikalikan
...
| Skenario | Desain saat ini | Satu runtime |
| --- | --- | --- |
| Satu server client, 5 modul | 611 MiB, 20 container, 6 database | 240 MiB, 5 container, 1 database |
| SaaS 100 tenant terisolasi: database | 501 | 100 |
| SaaS 100 tenant: RAM idle Postgres | sekitar 13 GiB | sekitar 2,6 GiB |
| Migrasi per rilis, 100 tenant | 500, lima jenis, harus urut | 100, satu jenis |
| 2.000 pengguna aktif: beban tambahan di Core | 300 request per detik dari lompatan nomor, periode, satuan | 0 |
| Latensi tambahan per dokumen | sekitar 30 ms, p95 40 ms | di bawah 1 ms |
| Worker PHP di puncak | sekitar 60, tiap app dipesan untuk puncaknya sendiri | sekitar 25, satu pool berbagi |
| Image per rilis | 11 | 1 |
...
| --- | --- | --- |
| Rumus | 191 + 5 × 84 | 191 + 5 × 2 MB + tabel modul di DB yang sama |
| RAM idle | sekitar 611 MiB | sekitar 240 MiB |
| Container | 5 + 5 × 3 = 20 | 5 |
| Database | 6 | 1 |
...
| RAM idle Postgres (26 MiB per database, angka lokal) | sekitar 13 GiB | sekitar 2,6 GiB |
...
2. Pindahkan management-aset masuk ke runtime Core sebagai modul pertama, dengan tabel berprefix, number sequence sebagai fungsi, dan test penjaga batas. Baris "proyeksi" di halaman ini diganti angka nyata setelah ini selesai.
