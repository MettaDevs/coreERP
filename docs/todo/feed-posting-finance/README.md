...
| K-29 | **Jurnal pembalikan penyusutan bertanggal periode asal.** `posting_date` dan `document_date` `asset.depreciation_reversal` sama dengan akhir periode yang dibalik, sehingga akun dan dimensinya terbaca sama dengan jurnal asalnya. Periode yang sudah tutup di aplikasi finance ditolak pembaca dan dikoreksi manual (K-17). | Keputusan pemilik produk, 24 September 2026, mengikuti D365: pembalikan transaksi aset tetap F&O bawaannya bertanggal sama dengan transaksi asalnya, dan tidak dapat dibalik ke periode yang sudah tutup. Baris pembalik di register memang bertanggal periode asal. |
...
| K-37 | **Presisi IDR: nilai 2 desimal, harga satuan 3 desimal** — melengkapi K-20. | Keputusan pemilik produk, 25 September 2026, tanpa konsultan. Kode old-finance membulatkan nilai ke dua desimal (`round(..., 2)`), jadi presisi pengirim sama dengan penerima dan tidak ada selisih pembulatan antara register, feed, dan old-finance. Data demo BC memakai 0 desimal untuk IDR, tetapi tidak dipilih: nilai bersen dari sistem lama harus dibulatkan lebih dulu. |
| K-38 | **Cutover tiap entitas legal jatuh di awal tahun buku berikutnya**: 1 Januari 2027 untuk entitas bertahun buku kalender. Tanggalnya tetap setelan per entitas legal di Core (K-16), bukan konstanta kode. | Keputusan pemilik produk, 25 September 2026. BO lama memakai satu `Tanggal Mulai System` dengan saldo awal akun neraca sehari sebelumnya, tanpa aturan kapan tanggal itu jatuh. F&O dan BC membuka saldo awal di awal periode fiskal, sehingga tahun berjalan tidak bercampur jurnal manual dan jurnal dari feed. |
| K-39 | **Akun posting group diisi langsung di layar posting group, tanpa templat berkas.** Nomor akun PPN Masukan, Hutang Usaha, dan penyeimbang saldo awal adalah data tenant yang dipilih dari daftar akun Core, bukan prasyarat kode. Tiap kolom akun membawa bantuan berisi contoh jurnalnya. | Keputusan pemilik produk, 25 September 2026, mengikuti BC: *FA Posting Groups*, *VAT Posting Setup*, dan *Vendor Posting Groups* diisi di layarnya masing-masing oleh tiap perusahaan, dan field description D365 muncul saat pointer diarahkan. Berkas templat terpisah akan basi begitu group aset bertambah. |
