...
Installer membaca module manifest. Pada SaaS ia memperoleh placement dari control plane; pada on-prem perpetual ia memakai manifest dan lisensi yang tersedia lokal. Ia selalu menjalankan langkah idempotent berikut:

```text
validate license/signature/version/dependency
-> choose deployment target
-> pull API and UI artifact
-> create or resolve module database
...
| Database | Resolve `module_pool_db` | Create/resolve `module_tenant_db` | Create volume/database module di Compose |
...
Satu monorepo tetap dipakai ketika terdapat banyak developer. Jumlah developer tidak mengubah boundary module, database, atau deployment. Yang berubah hanya kapasitas CI dan governance: developer tidak pernah mengunggah image atau bundle customer secara manual.
...
1. Pull request memicu test hanya untuk module/contract yang berubah dan consumer contract yang terdampak. CI boleh mengantri ketika banyak PR; antrean tidak mengubah hasil release.
...
Dengan aturan ini, sepuluh developer dapat bekerja paralel tanpa saling menimpa release. Bila volume job meningkat, kapasitas runner `ci-01` ditambah; bukan menambah jalur deploy, registry, atau proses install baru.
...
flowchart TD
    ...
    APPROVE -->|On-prem customer membeli atau upgrade module| EDITION[Sales/Ops: update Customer Edition Manifest]
...
| --- | --- | --- | --- |
| Pengembangan | Developer | Mengubah satu boundary aplikasi/module di branch, menulis test, membuka PR | Deploy manual ke Dev/Staging/Prod atau server customer. |
...
| On-prem | Sales/Ops, CI, operator customer | Memilih entitlement customer, menerbitkan bundle bertanda tangan, lalu customer meng-install lokal | Developer SSH ke server customer atau mengirim source/module tidak dibeli. |
...
## Customer Edition Manifest dan pembelian module tambahan
...
1. Operator mengunduh atau menerima media bundle release dan lisensi/module entitlement bertanda tangan.
2. Jalankan verifier lokal untuk signature/image digest dan cek compatibility matrix Core/module/dependency/database schema.
...
5. Aktifkan traffic setelah health check sukses; catat `module_installations` dan audit event lokal.
...
- Source module yang tidak dibeli harus absent dari on-prem bundle, Compose manifest, build context, dan image cache.
- Lisensi perpetual dan module entitlement diverifikasi secara lokal dari signature vendor. Tidak ada call-home atau online grace period sebagai dependency runtime. Perpanjangan support atau pembelian module baru menghasilkan bundle/lisensi bertanda tangan yang dipasang operator.
