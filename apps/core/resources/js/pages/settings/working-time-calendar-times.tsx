...
import { ArrowLeft, CalendarDays, Check, Clock, Play, RotateCcw, X } from 'lucide-react';
...
export default function WorkingTimeCalendarTimes({
    ...
    const selectedDay = useMemo(
        () => days.find((d) => d.id === selectedDayId) ?? (days.length > 0 ? days[0] : null),
        [days, selectedDayId],
    );
    ...
    const handleToggleDayControl = (day: CalendarDay) => {
        ...
        router.put(
            ...
            {
                control: newControl,
                closed_for_pickup: newControl === 'closed' ? true : day.closed_for_pickup,
            },
            {
                preserveScroll: true,
    ...
    return (
        <>
            <Head title={calendar ? `Jadwal Kerja - ${calendar.code}` : 'Jadwal Kerja'} />
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {allCalendars.length > 0 ? (
                            <div className="flex items-center gap-1.5">
                                <span className="font-medium text-muted-foreground">Pilih Kalender:</span>
                                <select
                                    value={calendar?.id ?? ''}
                                    onChange={(e) => {
                                        if (e.target.value) {
                                            router.visit(`/settings/working-time-calendar-times/${e.target.value}`);
                                        }
                                    }}
                                    className="h-7 rounded border border-input bg-background px-2 text-xs font-semibold text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                                >
                                    {allCalendars.map((c) => (
                            ...
                            <span className="text-xs text-muted-foreground">Belum ada kalender dibuat</span>
                        )}
                    ...
                    <div className="flex flex-1 items-center justify-center p-8">
                        <div className="max-w-md text-center">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                                Silakan buat kalender kerja terlebih dahulu di menu <strong>Kalender kerja</strong> sebelum melihat atau mengatur jadwal kerja.
                            </p>
                            <Link
                                href="/settings/working-time-calendars"
                    ...
                    <div className="flex flex-col rounded-md border border-border bg-card">
                        <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
                            <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                                Hari Kerja (Working Days) — Total: {days.length} hari
                            </span>
                            <span className="text-xs text-muted-foreground">
                                Klik baris untuk melihat rincian jam kerja di bawah
                            </span>
                        </div>
                            ...
                            <Table>
                                <TableHeader className="sticky top-0 bg-card">
                                    <TableRow>
                                        ...
                                        <TableHead className="w-20 text-center">Minggu</TableHead>
                                        <TableHead className="w-28">Bulan</TableHead>
                                        <TableHead className="w-28 text-center">Kontrol</TableHead>
                                        <TableHead className="w-36 text-center">Tutup Pengambilan</TableHead>
                                        <TableHead className="w-28 text-right">Total Jam</TableHead>
                                        {canManage && <TableHead className="w-24 text-center">Aksi</TableHead>}
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {days.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={9} className="py-8 text-center text-muted-foreground">
                                                Belum ada jadwal kerja untuk periode ini. Klik{' '}
                                                <strong>Buat jadwal dari pola (Compose)</strong> untuk meng-generate jadwal.
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        days.map((day) => {
                                            const isSelected = day.id === (selectedDay?.id ?? null);
                                            const dObj = new Date(`${day.date}T00:00:00`);
                                            const dayName = DAY_NAMES[day.day_of_week] || '-';
                                            const monthName = MONTH_NAMES[dObj.getMonth()] || '-';
                                            const weekNr = getWeekNumber(dObj);
                                            const isOpen = day.control === 'open';
                                            ...
                                            return (
                                                ...
                                                >
                                                    <TableCell className="text-center" onClick={(e) => e.stopPropagation()}>
                                                        <Checkbox
                                                            checked={isSelected}
                                                            onCheckedChange={() => setSelectedDayId(day.id)}
                                                        />
                                                    </TableCell>
                                                    ...
                                                    {canManage && (
                                                        <TableCell className="text-center" onClick={(e) => e.stopPropagation()}>
                                                            <Button
                                                                ...
                                                                size="sm"
                                                                type="button"
                                                                onClick={() => handleToggleDayControl(day)}
                    ...
                    <div className="flex flex-1 flex-col rounded-md border border-border bg-card">
                        <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
                            <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                                ...
                                ({selectedDay ? DAY_NAMES[selectedDay.day_of_week] : '-'})
                            </span>
                            {selectedDay && (
                                <span className="text-xs font-mono text-muted-foreground">
                                    Total: {selectedDay.hours.toFixed(2)} jam kerja
                                </span>
                            )}
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead className="w-32">Kalender</TableHead>
                                        <TableHead className="w-28">Dari Jam</TableHead>
                                        <TableHead className="w-28">Sampai Jam</TableHead>
                                        <TableHead className="w-32 text-right">Efisiensi (%)</TableHead>
                                        <TableHead>Properti</TableHead>
                                        <TableHead className="w-32 text-right">Durasi Jam</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {!selectedDay || selectedDay.lines.length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={6} className="py-6 text-center text-muted-foreground">
                                                Tidak ada jam kerja efektif pada hari ini (hari libur / ditutup).
                                            </TableCell>
                                        </TableRow>
                                    ) : (
                                        selectedDay.lines.map((line, idx) => (
                                            <TableRow key={line.id || idx}>
                                                ...
                                                <TableCell className="text-muted-foreground">
                                                    {line.property || 'Reguler'}
                                                </TableCell>
                                                <TableCell className="text-right font-mono text-xs font-semibold">
                                                    {line.hours.toFixed(2)} jam
                                                </TableCell>
                                            </TableRow>
                                        ))
                                    )}
            ...
            <Dialog open={isComposeOpen} onOpenChange={setIsComposeOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Buat Jadwal dari Pola (Compose Working Times)</DialogTitle>
                        <DialogDescription>
                            Generate jadwal hari dan jam kerja secara otomatis untuk rentang tanggal tertentu berdasarkan pola jam kerja yang dipilih.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogBody className="space-y-4 py-2">
                        <div>
                            ...
                            <Input
                                value={calendar ? `${calendar.code} - ${calendar.name}` : ''}
                                disabled
                        ...
                        <div>
                            <label className="text-xs font-semibold text-foreground">
                                Pola Jam Kerja (Template) <span className="text-destructive">*</span>
                            </label>
                            {templates.length === 0 ? (
                                <p className="mt-1 text-xs text-destructive">
                                    Belum ada template pola jam kerja yang aktif. Buat template terlebih dahulu di menu <strong>Pola jam kerja</strong>.
                                </p>
                            ) : (
                                <select
                            ...
                            {composeErrors.template_id && (
                                <p className="mt-1 text-xs text-destructive">{composeErrors.template_id}</p>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setComposeFromDate(e.target.value)}
                                    className="mt-1"
                                />
                                {composeErrors.from_date && (
                                    <p className="mt-1 text-xs text-destructive">{composeErrors.from_date}</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <Input
                                    type="date"
                                    value={composeToDate}
                                    onChange={(e) => setComposeToDate(e.target.value)}
                                    className="mt-1"
                                />
                                {composeErrors.to_date && (
                                    <p className="mt-1 text-xs text-destructive">{composeErrors.to_date}</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="rounded-md bg-muted/60 p-3 text-xs text-muted-foreground">
                            ...
                            <p className="mt-0.5">
                                Jadwal yang sudah ada pada rentang tanggal tersebut akan diperbarui sesuai pola jam kerja mingguan dari template yang dipilih.
                            </p>
                        </div>
                    </DialogBody>
                    ...
                    <DialogFooter>
                        <DialogAction
                            ...
                            disabled={isComposing || !composeTemplateId || !composeFromDate || !composeToDate}
                        >
...
WorkingTimeCalendarTimes.layout = {
    breadcrumbs: [
        ...
        { title: 'Jadwal kerja', href: '/settings/working-time-calendar-times' },
