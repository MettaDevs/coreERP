...
/**
 * Klien OIDC konsol operator ke penyedia identitas bersama.
 *
 * ## Salinan dari Core, dan itu disebut apa adanya
 *
 * Isinya sama dengan `App\Support\Sso\SharedIdentityProvider` milik Core; yang berbeda hanya kunci
 * config-nya (`sso.*`, klien "CoreERP Admin") dan kunci simpanannya. Konsol dan Core adalah dua
 * image terpisah — image edisi on-prem bahkan dijaga agar tidak memuat folder konsol — jadi tidak ada
 * tempat bersama untuk kode PHP di antara keduanya. Salinan yang sama dipilih konsol untuk
 * `EnvironmentAddress`, dengan alasan yang sama. Yang menahan keduanya tetap sepakat adalah test
 * penolakan yang sama di kedua suite: tanda tangan asing, `iss`, `aud`, `nonce`, dan masa berlaku.
 *
...
class IdentityProvider
{
    ...
    {
        $cacheKey = 'console.sso.jwks.'.sha1($this->issuer());
    ...
    {
        return Cache::remember('console.sso.discovery.'.sha1($this->issuer()), 3600, function (): array {
