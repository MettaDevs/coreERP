...
class UnitOfMeasureServiceTest extends TestCase
{
    ...
    {
        $tenant = (string) Str::ulid(); $class = (string) Str::ulid(); $system = (string) Str::ulid(); $kg = (string) Str::ulid(); $gram = (string) Str::ulid(); $now = now();
        ...
        foreach ([[$kg, 'KG', 'Kilogram'], [$gram, 'G', 'Gram']] as [$id, $code, $name]) DB::table('units_of_measure')->insert(['id' => $id, 'tenant_id' => $tenant, 'uom_class_id' => $class, 'uom_system_id' => $system, 'code' => $code, 'name' => $name, 'decimal_places' => 3, 'active' => true, 'created_at' => $now, 'updated_at' => $now]);
