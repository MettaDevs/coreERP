...
import { Users, ArrowLeftRight, HelpCircle, ChevronUp, ChevronDown } from 'lucide-react';
import type { RelationshipItem, RelationshipDirection } from '@/types/global-address-book';
                ...
                {isExpanded && (
                    ...
                    <div className="flex flex-col gap-4 rounded-lg border bg-muted/20 p-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="w-full sm:w-72">
                            <Select
                                ...
                                items={[
                                    { value: 'a_to_b', label: 'Tampilan: Perspektif A ke B' },
                                    { value: 'b_to_a', label: 'Tampilan: Perspektif B ke A (Kebalikan)' },
                                ]}
                                value={direction}
                                onValueChange={(val) => setDirection((val as RelationshipDirection) || 'a_to_b')}
                            />
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm">
                            ...
                            <label className="flex items-center gap-1.5 cursor-pointer">
                                <Checkbox
                                    ...
                                    onCheckedChange={(checked) => setFilterExpired(!!checked)}
                                />
                                <span className="text-xs">Kedaluwarsa (Expired)</span>
                            </label>
                            ...
                            <label className="flex items-center gap-1.5 cursor-pointer">
                                ...
                                />
                                <span className="text-xs">Aktif (Active)</span>
                            </label>

                            <label className="flex items-center gap-1.5 cursor-pointer">
                                <Checkbox
                                    checked={filterFuture}
                                    onCheckedChange={(checked) => setFilterFuture(!!checked)}
                                />
                                <span className="text-xs">Masa Depan (Future)</span>
                            </label>

                            <button
                                type="button"
                                onClick={() => setShowExplanation(!showExplanation)}
                                className="ml-auto flex h-6 w-6 items-center justify-center rounded-sm text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
                        ...
                        <div className="rounded-lg border bg-card p-4 shadow-xs">
                            ...
                            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 text-xs">
                                <div className="rounded-md bg-muted/40 p-2.5">
                                    <span className="font-semibold text-foreground">Dari Party B ke A</span>
                                ...
                                <div className="rounded-md bg-muted/40 p-2.5">
                                    <span className="font-semibold text-foreground">Kebalikannya dari A ke B</span>
                                    <ul className="mt-1 list-disc pl-4 space-y-1 text-muted-foreground">
                                        <li>Memiliki PIC</li>
                    ...
                    {filteredRelationships.length === 0 ? (
                        <Empty className="py-8">
                            ...
                            <EmptyHeader>
                                <EmptyTitle>Belum ada hubungan relasi tercatat</EmptyTitle>
                                <EmptyDescription>
                                    Hubungkan pihak ini dengan organisasi atau individu lain sebagai cabang, PIC, atau mitra bisnis.
                                </EmptyDescription>
                            </EmptyHeader>
                        </Empty>
                    ) : (
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-muted/40">
                                        ...
                                        <TableHead>{direction === 'a_to_b' ? 'Party A (Sumber)' : 'Party B (Tujuan)'}</TableHead>
                                        <TableHead className="text-center">
                                            {direction === 'a_to_b' ? 'Hubungan (A ➔ B)' : 'Hubungan (B ➔ A)'}
                                        </TableHead>
                                        <TableHead>{direction === 'a_to_b' ? 'Party B (Tujuan)' : 'Party A (Sumber)'}</TableHead>
                                ...
                                <TableBody>
                                    {filteredRelationships.map((rel, idx) => {
                                        ...
                                        const relText = direction === 'a_to_b' ? rel.relationship_a_to_b : rel.relationship_b_to_a;
                                        const firstParty = direction === 'a_to_b' ? rel.party_a_name : rel.party_b_name;
                                        const secondParty = direction === 'a_to_b' ? rel.party_b_name : rel.party_a_name;
                                            ...
                                            >
                                                ...
                                                <TableCell className="font-medium">
                                                    ...
                                                    <span className="block text-xs font-mono text-muted-foreground">
                                                        {direction === 'a_to_b' ? rel.party_a_id : rel.party_b_id}
                                                    </span>
                                                </TableCell>
                                                ...
                                                <TableCell className="font-medium">
                                                    {secondParty}
                                                    <span className="block text-xs font-mono text-muted-foreground">
                                                        {direction === 'a_to_b' ? rel.party_b_id : rel.party_a_id}
                                                    </span>
                                                </TableCell>
                                                ...
                                                <TableCell className="text-xs text-muted-foreground">
                                                    {rel.expiration_date || 'Selamanya (Never)'}
                                                </TableCell>
                                                <TableCell className="text-center">
                                                    {rel.status === 'active' && (
