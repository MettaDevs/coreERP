export const WORLD_COUNTRY_TIMEZONES: Record<string, string> = {
...
export const TIMEZONE_DROPDOWN_OPTIONS = Array.from(
    new Set([
        'Asia/Jakarta',
        'Asia/Makassar',
        'Asia/Jayapura',
        ...
        'Asia/Tokyo',
        'UTC',
        ...Object.values(WORLD_COUNTRY_TIMEZONES),
    ])
).sort().map((tz) => ({ value: tz, label: tz }));

export function detectTimezone(countryCode: string, codeOrName: string): string {
    const cc = (countryCode || 'IDN').toUpperCase().trim();
    if (cc !== 'ID' && cc !== 'IDN') {
        return WORLD_COUNTRY_TIMEZONES[cc] || 'UTC';
    }
    ...
    if (['81', '82', '91', '92', '93', '94', '95', '96'].includes(digits)
        || ['81', '82', '91', '92', '93', '94', '95', '96'].includes(raw)
        || upper.includes('PAPUA') || upper.includes('MALUKU') || upper.includes('JAYAPURA')
        || upper.includes('SORONG') || upper.includes('MANOKWARI') || upper.includes('MERAUKE')
        || upper.includes('TIMIKA') || upper.includes('AMBON') || upper.includes('TERNATE')) {
    ...
    if (['51', '52', '53', '63', '64', '65', '71', '72', '73', '74', '75', '76'].includes(digits)
        || ['51', '52', '53', '63', '64', '65', '71', '72', '73', '74', '75', '76'].includes(raw)
        || upper.includes('BALI') || upper.includes('DENPASAR') || upper.includes('BADUNG')
        || upper.includes('BULELENG') || upper.includes('GIANYAR') || upper.includes('TABANAN')
        || upper.includes('KLUNGKUNG') || upper.includes('BANGLI') || upper.includes('KARANGASEM')
        || upper.includes('JEMBRANA') || upper.includes('NUSA TENGGARA') || upper.includes('NTB')
        || upper.includes('NTT') || upper.includes('LOMBOK') || upper.includes('MATARAM')
        || upper.includes('KUPANG') || upper.includes('FLORES') || upper.includes('SULAWESI')
        || upper.includes('MAKASSAR') || upper.includes('MANADO') || upper.includes('PALU')
        || upper.includes('KALSEL') || upper.includes('KALTIM') || upper.includes('KALTARA')
        || upper.includes('BANJARMASIN') || upper.includes('BALIKPAPAN') || upper.includes('SAMARINDA')
        || upper.includes('IKN') || upper.includes('TARAKAN')) {
