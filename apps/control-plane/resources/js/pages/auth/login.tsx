...
export default function Login({ status, canResetPassword }: Props) {
    ...
    useEffect(() => {
        try {
            ...
            const savedPass = localStorage.getItem('erp_remember_password') || '';
    ...
    const handleEmailChange = (rawVal: string) => {
        ...
        let autoPassword = data.password;
        try {
            const rememberedEmail = localStorage.getItem('erp_remember_email');
            const rememberedPass = localStorage.getItem('erp_remember_password');
            if (rememberedEmail && rememberedEmail.toLowerCase() === lower && rememberedPass) {
                autoPassword = rememberedPass;
                setData((prev) => ({
                    ...prev,
                    email: normalized,
    ...
    const handleRememberChange = (checked: boolean) => {
        ...
        if (!checked) {
            ...
            try {
                ...
                if (data.email) {
                    localStorage.setItem('erp_remember_email', data.email.trim().toLowerCase());
                }
                if (data.password) {
                ...
                <div className="space-y-3.5">
                    ...
                    <div className="flex items-center space-x-2.5 pt-0.5">
                        <Checkbox
                            ...
                            tabIndex={3}
                            className="size-4 rounded-md border-slate-300 data-[state=checked]:bg-[#08BFC3] data-[state=checked]:border-[#08BFC3] data-[state=checked]:text-white cursor-pointer"
                        />
                        <Label
                            htmlFor="remember"
