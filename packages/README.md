...
Core memakainya sebagai workspace npm — akar workspace adalah akar repo, dan `apps/control-plane` menyebutnya `"@apperp/ui": "*"`. Karena itu `npm ci` dijalankan dari akar repo, bukan dari folder app, dan hanya ada satu `package-lock.json` untuk keduanya. `npm run build` di Core membangun paket ini lebih dulu lewat skrip `ui:build`.
