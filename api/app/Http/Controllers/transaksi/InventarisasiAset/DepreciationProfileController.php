...
class DepreciationProfileController extends Controller
{
    ...
    public function store(Request $request, NumberSequenceClient $numbers): JsonResponse
    {
        ...
        if ($existing = DepreciationProfile::withTrashed()->where(['tenant_id' => $tenant, 'creation_key' => $key])->first()) {
            return response()->json(['data' => $this->present($existing)], 200, ['Idempotent-Replayed' => 'true']);
        }
        try {
            $kode = $numbers->issue('management-aset.profil-penyusutan', $tenant, self::RESOURCE.':'.$key);
    ...
    {
        if (in_array($data['method'], ['straight_line', 'reducing_balance'], true) && empty($data['useful_life_periods'])) {
            abort(422, 'Masa manfaat wajib diisi untuk metode ini.');
        }
        if ($data['method'] === 'reducing_balance' && empty($data['rate_percent'])) {
            abort(422, 'Persentase wajib diisi untuk saldo menurun.');
        }
        if ($data['method'] === 'manual' && empty($data['manual_schedule'])) {
            abort(422, 'Jadwal manual wajib diisi.');
        }
    ...
    {
        abort_unless(in_array('management-aset.'.self::RESOURCE.'.'.$action, $r->attributes->get('coreerp.permissions', []), true), 403);
    }
    ...
    private function present(DepreciationProfile $p): array
    {
        return $p->only(['id', 'kode', 'nama', 'method', 'frequency', 'year_basis', 'convention', 'useful_life_periods', 'rate_percent', 'manual_schedule', 'aktif']);
    }
}
