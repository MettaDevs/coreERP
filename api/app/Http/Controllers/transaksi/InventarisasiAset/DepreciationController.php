...
class DepreciationController extends Controller
{
    ...
    {
        $this->can($request, 'read'); $tenant = $this->tenant($request);
        $query = DB::table('tr_penyusutan_aset as period')->join('tr_buku_aset as book', function ($join): void { $join->on('book.id', '=', 'period.asset_book_id')->on('book.tenant_id', '=', 'period.tenant_id'); })->join('tr_penerimaan_aset as asset', function ($join): void { $join->on('asset.id', '=', 'book.asset_id')->on('asset.tenant_id', '=', 'book.tenant_id'); })->where('period.tenant_id', $tenant);
    ...
    {
        $this->can($request, 'read'); $tenant = $this->tenant($request);
        $query = DB::table('tr_buku_aset as book')->join('tr_penerimaan_aset as asset', function ($join): void { $join->on('asset.id', '=', 'book.asset_id')->on('asset.tenant_id', '=', 'book.tenant_id'); })->join('m_profil_penyusutan as profile', function ($join): void { $join->on('profile.id', '=', 'book.depreciation_profile_id')->on('profile.tenant_id', '=', 'book.tenant_id'); })->where(['book.tenant_id' => $tenant, 'book.status' => 'active']);
    ...
    {
        $this->can($request, 'create'); $tenant = $this->tenant($request);
        ...
        $query = DB::table('tr_buku_aset as book')->join('m_profil_penyusutan as profile', function ($join): void { $join->on('profile.id', '=', 'book.depreciation_profile_id')->on('profile.tenant_id', '=', 'book.tenant_id'); })->join('tr_penerimaan_aset as asset', function ($join): void { $join->on('asset.id', '=', 'book.asset_id')->on('asset.tenant_id', '=', 'book.tenant_id'); })->where(['book.tenant_id' => $tenant, 'book.id' => $data['asset_book_id']]);
        app(OrganizationScope::class)->assetQuery($query, $request, 'asset');
        $book = $query->select('book.*', 'profile.method', 'profile.frequency', 'profile.useful_life_periods', 'profile.rate_percent', 'profile.manual_schedule', 'asset.legal_entity_id', 'asset.responsible_org_unit_id')->first();
        ...
        $manualIndex = DB::table('tr_penyusutan_aset')->where(['tenant_id' => $tenant, 'asset_book_id' => $book->id])->whereNull('reverses_period_id')->whereDate('period_ends_on', '<', $data['period_ends_on'])->count();
        $amount = $this->amount($book, $data['consumption_amount'] ?? null, $manualIndex);
    ...
    {
        $this->can($request, 'finalize'); $tenant = $this->tenant($request);
        ...
        $result = DB::transaction(function () use ($tenant, $id, $request, $scope): array {
            ...
            $period = $query->lockForUpdate()->first(); abort_unless($period, 404); abort_if($period->status === 'final', 409, 'Periode penyusutan sudah final.');
            DB::table('tr_penyusutan_aset')->where('id', $id)->update(['status' => 'final', 'updated_at' => now()]);
            $book = DB::table('tr_buku_aset as book')->join('tr_penerimaan_aset as asset', function ($join): void { $join->on('asset.id', '=', 'book.asset_id')->on('asset.tenant_id', '=', 'book.tenant_id'); })->where(['book.tenant_id' => $tenant, 'book.id' => $period->asset_book_id])->select('book.*', 'asset.kode as asset_code', 'asset.currency_code')->first();
            DB::table('tr_buku_aset')->where('id', $book->id)->update(['accumulated_depreciation' => DB::raw('accumulated_depreciation + '.(float) $period->amount), 'net_book_value' => DB::raw('net_book_value - '.(float) $period->amount), 'updated_at' => now()]);
    ...
    {
        $this->can($request, 'correct'); $tenant = $this->tenant($request);
        ...
        $result = DB::transaction(function () use ($tenant, $id, $data, $request, $scope): array {
            ...
            abort_unless($original, 404); abort_unless($original->status === 'final', 409, 'Hanya periode final yang dapat dibalik.');
            ...
            $book = DB::table('tr_buku_aset as book')->join('tr_penerimaan_aset as asset', function ($join): void { $join->on('asset.id', '=', 'book.asset_id')->on('asset.tenant_id', '=', 'book.tenant_id'); })->where(['book.tenant_id' => $tenant, 'book.id' => $original->asset_book_id])->select('book.*', 'asset.kode as asset_code', 'asset.currency_code')->lockForUpdate()->first();
            abort_unless($book, 404);
            ...
            DB::table('tr_penyusutan_aset')->insert($period);
            DB::table('tr_buku_aset')->where('id', $book->id)->update(['accumulated_depreciation' => DB::raw('accumulated_depreciation - '.(float) $original->amount), 'net_book_value' => DB::raw('net_book_value + '.(float) $original->amount), 'updated_at' => now()]);
    ...
    private function amount(object $book, ?float $consumption, int $manualIndex): float
    {
        $net = max(0, (float) $book->net_book_value - (float) $book->residual_value);
        return match ($book->method) {
            'straight_line' => round(((float) $book->acquisition_value - (float) $book->residual_value) / max(1, (int) $book->useful_life_periods), 2),
            'reducing_balance' => round($net * ((float) $book->rate_percent / 100) / $this->periodsPerYear($book->frequency), 2),
            'consumption' => round((float) ($consumption ?? 0), 2),
            'manual' => $this->manualAmount($book->manual_schedule, $manualIndex),
            default => 0,
        };
    }
    private function manualAmount(?string $schedule, int $index): float
    {
        $item = json_decode($schedule ?: '[]', true)[$index] ?? null;
        abort_unless(is_array($item) && array_key_exists('amount', $item), 422, 'Jadwal manual belum memiliki nilai untuk periode ini.');
        return round((float) $item['amount'], 2);
    }
    private function periodsPerYear(string $frequency): int { return ['monthly' => 12, 'quarterly' => 4, 'half_yearly' => 2, 'yearly' => 1][$frequency] ?? 12; }
    private function can(Request $r, string $action): void { abort_unless(in_array('management-aset.penyusutan.'.$action, $r->attributes->get('coreerp.permissions', []), true), 403); }
