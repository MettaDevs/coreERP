...
final class UnitOfMeasureClient
{
    ...
    public function resolve(string $tenantId, array $ids): array
    {
        ...
        if (! is_array($units) || count($units) !== count($ids)) throw new RuntimeException('Satuan belum dapat divalidasi.');
        return collect($units)->keyBy('id')->all();
    }

    private function request(string $tenantId, string $method, string $path, array $payload = []): \Illuminate\Http\Client\Response
    {
        $url = rtrim((string) config('services.coreerp.url'), '/'); $token = (string) config('services.coreerp.service_token');
        if ($url === '' || $token === '') throw new RuntimeException('Layanan satuan belum dikonfigurasi.');
        try {
            ...
            report($exception); throw new RuntimeException('Satuan belum dapat dihubungi.', previous: $exception);
        }
    }
}
