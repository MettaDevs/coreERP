...
- Sebelum menyentuh `apps/control-plane` (admin.erp), kontrak API agen, tabel milik sisi pusat, autentikasi operator, atau SSO di kedua aplikasi, buka skill `coreerp-konsol`. Konteksnya ada di [`docs/dev/31-admin-erp-konsol.md`](docs/dev/31-admin-erp-konsol.md) dan [`docs/dev/32-sso.md`](docs/dev/32-sso.md). Konsol sengaja tidak punya migration: tabel baru miliknya lahir di Core, beserta model penanda `OwnedByControlPlane`.
