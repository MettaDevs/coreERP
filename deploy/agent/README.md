...
`finance_feed` — jumlah posting finance per status, jam terbit posting `pending` tertua, jam pull terakhir, dan
jam push terakhir yang diterima pembaca — satu-satunya kunci yang datang dari Core. `baca_ringkasan_feed` menjalankan
...
bukan angka nol, dan laporannya tetap terkirim. `last_pushed_at` hanya ikut bila keluaran Core menyebutnya: rilis
Core yang lebih lama tidak mengenalnya, dan `null` berarti "belum pernah ada push yang diterima" — klaim yang tidak
boleh dibuat atas nama Core itu. Pengujian `09d` membuktikan penyusunan ulangnya, `null`-nya, kunci push yang
opsional, dan batas waktunya.
