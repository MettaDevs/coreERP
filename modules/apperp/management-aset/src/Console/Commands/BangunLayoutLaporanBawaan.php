...
class BangunLayoutLaporanBawaan extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->assetDisposalSaleReportXlsx($laporan.'/laporan-penjualan-aset/standar.xlsx');
    ...
    private function assetDisposalSaleReportXlsx(string $path): void
    {
        $spreadsheet = new Spreadsheet;
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Penjualan aset');

        // Kop surat Core: kolom A sampai K (11 kolom)
        $sheet->setCellValue('A1', '${kop.logo_kiri}');
        $sheet->mergeCells('A1:A3');
        $sheet->setCellValue('B1', '${kop.induk}');
        ...
        $sheet->setCellValue('K1', '${kop.logo_kanan}');
        $sheet->mergeCells('K1:K3');
        $sheet->getStyle('B1:J3')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->getStyle('B2')->getFont()->setBold(true)->setSize(13);
        $sheet->getRowDimension(1)->setRowHeight(22);
        $sheet->getRowDimension(2)->setRowHeight(24);
        $sheet->getRowDimension(3)->setRowHeight(22);
        ...
        $sheet->setCellValue('A8', 'Jumlah penjualan');
        $sheet->setCellValue('B8', '${jumlah_penjualan}');
        $sheet->setCellValue('D8', 'Total nilai penjualan');
        $sheet->setCellValue('E8', '${total_nilai_penjualan}');
        $sheet->setCellValue('G8', 'Dicetak');
        $sheet->setCellValue('H8', '${dicetak_pada}');

        $headings = [
            'No', 'No. Bukti', 'Tgl Penjualan', 'Kode Aset', 'Item Aset', 'Spesifikasi',
            'Nilai Penjualan', 'Nilai Buku', 'Laba / Rugi', 'Keterangan', 'Status Dokumen',
        ];
        $macros = [
            'nomor', 'no_bukti', 'tanggal_penjualan', 'asset_kode', 'asset_nama', 'spesifikasi',
            'nilai_penjualan', 'nilai_buku', 'laba_rugi', 'keterangan', 'status_dokumen',
        ];

        $columnWidths = [
            'nomor' => 6,
            'no_bukti' => 18,
            'tanggal_penjualan' => 16,
            'asset_kode' => 16,
            'asset_nama' => 22,
            'spesifikasi' => 22,
            'nilai_penjualan' => 20,
        ...
        $header = $sheet->getStyle('A10:K10');
        $header->getFont()->setBold(true);
        $header->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('E7E6E6');
        $header->getBorders()->getBottom()->setBorderStyle(Border::BORDER_THIN);
        $header->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
        $sheet->getStyle('A11')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->getStyle('G11:I11')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
