 ...
 * adalah perutean hash di dalamnya. Hash itu ada karena satu image harus bisa disajikan di
 * bawah awalan penempatan mana pun; setelah UI menyatu dengan shell, awalan itu tidak ada
 * lagi dan alamatnya menjadi alamat biasa: `/management-aset/<id menu>/<ruas milik layar>`.
 *
 * **Kenapa satu controller, bukan satu per menu.** Ketiga puluh tiga entri menu memakai
 * rangkaian percabangan yang sama di sisi React, dan memecahnya menjadi 33 controller berarti
 * 33 tempat yang harus diubah setiap kali bentuk propertinya berubah. Yang membedakan layar
 * satu dengan lainnya hanya id menunya, dan id itu sudah ada di `app.yaml`.
 *
...
final class HalamanModulController
{
    ...
    public function __invoke(Request $request, KonteksPermintaan $akses, string $view): Response
    {
        ...
        return Inertia::render('management-aset::Modul', [
            ...
            // `/management-aset/pemeliharaan-aset/01JQ…/ubah`. Layar yang membuka satu record
            // pada halaman tersendiri memakainya, sehingga tombol kembali peramban, muat ulang,
            // dan tautan yang disalin semuanya mendarat di record yang sama — persis yang dulu
            // dikerjakan ruas-ruas sesudah hash.
            'segments' => self::ruas($request, $view),
     ...
     * @return list<string>
     */
    private static function ruas(Request $request, string $view): array
    {
